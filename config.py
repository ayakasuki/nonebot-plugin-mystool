...
Config.add_plugin_config(
    ...
    "00:30",
    name="每日自动签到时间",
    help_="每日自动签到和米游社任务的定时任务执行时间，格式为HH:MM",
    default_value = 10
)
Config.add_plugin_config(
    "mysTool",
    ...
    "米游社小助手",
    name="插件内部命令头",
    help_="插件内部命令头(若为""空字符串则不启用),默认米游社小助手",
    default_value = "米游社小助手"
)
Config.add_plugin_config(
    "mysTool",
    "ADD_FRIEND_ACCEPT",
...
Config.add_plugin_config(
    ...
    False,
    name="自动同意好友申请",
    help_="用户添加机器人为好友以后，是否发送使用该插件指引信息,默认否",
    default_value = False
)
Config.add_plugin_config(
    "mysTool",
...
Config.add_plugin_config(
    "mysTool",
    "RESIN_CHECK_INTERVAL",
    60,
    name="每次检查原神便笺间隔",
    help_="每次检查原神便笺间隔，单位为分钟,默认60",
    default_value = 60
)
Config.add_plugin_config(
    "mysTool",
    "NTP_SERVER",
    "ntp.aliyun.com",
    name="NTP服务器，用于获取网络时间",
    help_="NTP服务器，用于获取网络时间",
    default_value = "ntp.aliyun.com"
)
Config.add_plugin_config(
    "mysTool",
    "MAX_RETRY_TIMES",
...
Config.add_plugin_config(
    ...
    "SLEEP_TIME",
    5,
    name="任务操作冷却时间(如米游币任务)",
    help_="任务操作冷却时间(如米游币任务)，默认5",
    default_value = 5
)
Config.add_plugin_config(
    "mysTool",
...
Config.add_plugin_config(
    ...
    "https://ghproxy.com/",
    name="网络请求超时时间",
    help_="GitHub代理加速服务器(若为""空字符串则不启用)",
    default_value = "https://ghproxy.com/"
)
...
class Config(BaseModel, extra=Extra.ignore):
    ...
    MAX_USER: int = Config.get_config("mysTool", "MAX_USER")
    '''支持最多用户数'''
    ADD_FRIEND_ACCEPT: bool = Config.get_config("mysTool", "ADD_FRIEND_ACCEPT")
    '''是否自动同意好友申请'''
    ADD_FRIEND_WELCOME: bool = Config.get_config("mysTool", "ADD_FRIEND_WELCOME")
    '''用户添加机器人为好友以后，是否发送使用指引信息'''
    COMMAND_START: str = Config.get_config("mysTool", "COMMAND_START")
    '''插件内部命令头(若为""空字符串则不启用)'''
    ...
    NTP_SERVER: str = Config.get_config("mysTool", "NTP_SERVER")
    '''NTP服务器，用于获取网络时间'''
    MAX_RETRY_TIMES: int = Config.get_config("mysTool", "MAX_RETRY_TIMES")
    '''网络请求失败后最多重试次数'''
    SLEEP_TIME: float = Config.get_config("mysTool", "SLEEP_TIME")
    '''任务操作冷却时间(如米游币任务)'''
    SLEEP_TIME_RETRY: float = Config.get_config("mysTool", "SLEEP_TIME_RETRY")
    '''网络请求出错的重试冷却时间'''
    TIME_OUT: Optional[float] = Config.get_config("mysTool", "TIME_OUT")
    '''网络请求超时时间'''
    GITHUB_PROXY: str = Config.get_config("mysTool", "GITHUB_PROXY")
    '''GitHub代理加速服务器(若为""空字符串则不启用)'''

    SIGN_TIME: str = Config.get_config("mysTool", "SIGN_TIME")
    '''每日自动签到和米游社任务的定时任务执行时间，格式为HH:MM'''
    RESIN_CHECK_INTERVAL: int = Config.get_config("mysTool", "RESIN_CHECK_INTERVAL")
    '''每次检查原神便笺间隔，单位为分钟'''
    EXCHANGE_THREAD: int = Config.get_config("mysTool", "EXCHANGE_THREAD")
    '''商品兑换线程数'''
