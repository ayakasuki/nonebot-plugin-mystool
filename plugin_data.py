...
class Preference(BaseSettings):
    ...
    偏好设置
    """
    github_proxy: Optional[str] = Config.get_config("mysTool", "GITHUB_PROXY")
    """GitHub加速代理 最终会拼接在原GitHub链接前面"""
    enable_connection_test: bool = True
    """是否开启连接测试"""
    connection_test_interval: Optional[float] = 30
    """连接测试间隔（单位：秒）"""
    timeout: float = Config.get_config("mysTool", "TIME_OUT")
    """网络请求超时时间（单位：秒）"""
    max_retry_times: Optional[int] = Config.get_config("mysTool", "MAX_RETRY_TIMES")
    """最大网络请求重试次数"""
    retry_interval: float = Config.get_config("mysTool", "SLEEP_TIME_RETRY")
    """网络请求重试间隔（单位：秒）（除兑换请求外）"""
    enable_ntp_sync: Optional[bool] = True
    """是否开启NTP时间同步（将调整实际发出兑换请求的时间，而不是修改系统时间）"""
    ntp_server: Optional[str] = Config.get_config("mysTool", "NTP_SERVER")
    """NTP服务器地址"""
    timezone: Optional[str] = "Asia/Shanghai"
    """兑换时所用的时区"""
    exchange_thread_count: int = Config.get_config("mysTool", "EXCHANGE_THREAD")
    """兑换线程数"""
    ...
    '''任务操作冷却时间(如米游币任务)'''
    plan_time: str = Config.get_config("mysTool", "SIGN_TIME")
    '''每日自动签到和米游社任务的定时任务执行时间，格式为HH:MM'''
    resin_interval: int = Config.get_config("mysTool", "RESIN_CHECK_INTERVAL")
    '''每次检查原神便笺间隔，单位为分钟'''
